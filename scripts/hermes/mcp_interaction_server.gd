...
# --- Hermes OS Ping ---
func _cmd_os_ping() -> void:
	var now := Time.get_datetime_dict_from_system()
	var timestamp: String = "%04d-%02d-%02dT%02d:%02d:%02d" % [now.year, now.month, now.day, now.hour, now.minute, now.second]
	_send_response({
		"success": true,
		"pong": true,
		"timestamp": timestamp,
...
# --- Hermes OS Observe ---
func _cmd_os_observe(params: Dictionary) -> void:
	_send_response({
		"success": true,
		"observe": _build_os_observe_payload(params)
	})


func _cmd_os_get_ui_tree(params: Dictionary) -> void:
	_send_response({
		"success": true,
		"ui_tree": _build_os_ui_tree_payload(params)
	})


func _cmd_ui_click(params: Dictionary) -> void:
	var ref: String = str(params.get("ref", "")).strip_edges()
	if ref == "":
		_send_response({
			"success": false,
			"click": {
				...
				"unsupported": true,
				"code": "MISSING_REF",
				"message": "ui_click requires non-empty ref",
				"ref": ref
			}
		})
		return
	...
	if shell == null:
		_send_response({
			...
			"click": {
				...
				"message": "Hermes shell not found",
				"ref": ref
			},
			"focused_window": {},
	...
	var click_result: Dictionary = _perform_os_ui_ref_click(shell, ref)
	var response: Dictionary = {
		"success": bool(click_result.get("ok", false)),
		"click": click_result,
		"focused_window": _focused_window_summary(shell),
		"observe": _build_os_observe_payload({})
	}
...
func _cmd_os_read_file(params: Dictionary) -> void:
	_send_os_file_operation_response("files.read_file", params)


func _cmd_os_write_file(params: Dictionary) -> void:
	_send_os_file_operation_response("files.write_file", params)
...
func _send_os_file_operation_response(operation: String, params: Dictionary) -> void:
	...
	var operation_result: Dictionary = result_value if result_value is Dictionary else {
		"ok": false,
		"error": {"code": "BAD_RESULT", "message": "Hermes operation router returned a non-dictionary result"},
		"result": {},
		"operation": operation
	}
	var response: Dictionary = {
...
func _build_os_ui_tree_payload(_params: Dictionary = {}) -> Array:
	...
	var launcher_node := _build_launcher_ui_tree_node(shell)
	if not launcher_node.is_empty():
		tree_items.append(launcher_node)

	var taskbar_node := _build_taskbar_ui_tree_node(shell)
	if not taskbar_node.is_empty():
		tree_items.append(taskbar_node)

	for window in _visible_windows_for_ui_tree(shell):
		if window == null or not is_instance_valid(window):
			continue
		if not bool(window.visible):
			continue
		var window_node: Dictionary = _build_window_ui_tree_node(shell, window)
		if not window_node.is_empty():
			tree_items.append(window_node)

	var sanitized: Variant = _variant_to_json(tree_items)
	if sanitized is Array:
		return sanitized as Array
	return []


func _build_launcher_ui_tree_node(shell: Node) -> Dictionary:
	var launcher_variant: Variant = shell.get("_launcher")
	if not (launcher_variant is Control):
		return {}
	var launcher: Control = launcher_variant as Control
	if not launcher.visible:
		return {}

	var app_order_variant: Variant = shell.get("_app_order")
	var app_order: Array = app_order_variant if app_order_variant is Array else []
	var launcher_buttons_variant: Variant = shell.get("_launcher_buttons")
	if not (launcher_buttons_variant is Dictionary):
		return {}
	var launcher_buttons: Dictionary = launcher_buttons_variant

	var children: Array = []
	for app_id_variant in app_order:
		var app_id: String = str(app_id_variant)
		if not launcher_buttons.has(app_id):
			continue
		var button_variant: Variant = launcher_buttons[app_id]
		if not (button_variant is Button):
			continue
		var button: Button = button_variant as Button
		if not _is_clickable_control_visible(button):
			continue
		children.append({
			"ref": "shell.launcher.app.%s" % app_id,
			"type": "button",
...
func _build_taskbar_ui_tree_node(shell: Node) -> Dictionary:
	var task_buttons_variant: Variant = shell.get("_task_buttons")
	if not (task_buttons_variant is Dictionary):
		return {}
	var task_buttons: Dictionary = task_buttons_variant
	if task_buttons.is_empty():
		return {}

	var children: Array = []
	for app_id_variant in task_buttons.keys():
		var app_id: String = str(app_id_variant)
		var button_variant: Variant = task_buttons[app_id_variant]
		if not (button_variant is Button):
			continue
		var button: Button = button_variant as Button
		if not _is_clickable_control_visible(button):
			continue
		children.append({
			"ref": "app:%s.taskbar.toggle" % app_id,
			"type": "button",
			"label": _button_label(button),
			"clickable": true,
			"enabled": not button.disabled,
...
func _visible_windows_for_ui_tree(shell: Node) -> Array:
	if shell.has_method("_all_open_windows_in_app_order"):
		var windows_value: Variant = shell.call("_all_open_windows_in_app_order")
		if windows_value is Array:
			return windows_value as Array

	var fallback: Array = []
	var open_windows_value: Variant = shell.get("_open_windows")
	if open_windows_value is Dictionary:
		var open_windows: Dictionary = open_windows_value
		for key in open_windows.keys():
			fallback.append(open_windows[key])
	return fallback


func _build_window_ui_tree_node(shell: Node, window: Node) -> Dictionary:
	var window_id: String = _window_id_for_shell(shell, window)
	if window_id == "":
		window_id = "win_%s" % str(window.get_instance_id())
	var app_id: String = str(window.get("app_id"))
	var title: String = str(window.get("app_title"))
	var focused: bool = _is_window_focused(shell, window)

	var children: Array = [
		_window_action_ref(window_id, app_id, title, focused, "window.focus", "Focus window"),
		_window_action_ref(window_id, app_id, title, focused, "window.close", "Close window")
	]
	children.append_array(_window_app_specific_actions(window, window_id, app_id, title, focused))

	return {
		"ref": "%s:window" % window_id,
		"type": "window",
		"label": title,
		"clickable": false,
		"enabled": true,
...
func _window_action_ref(window_id: String, app_id: String, title: String, focused: bool, suffix: String, label: String) -> Dictionary:
	return {
		"ref": "%s:%s" % [window_id, suffix],
		"type": "action",
		"label": label,
		"clickable": true,
		"enabled": true,
...
func _window_app_specific_actions(window: Node, window_id: String, app_id: String, title: String, focused: bool) -> Array:
	var actions: Array = []
	if app_id == "files":
		var files_root := window.find_child("FilesRoot", true, false)
		if files_root != null:
			actions.append_array(_button_action_refs(files_root, window_id, app_id, title, focused, {
				"New file": "files.new_file",
				"Open": "files.open",
				"Delete": "files.delete"
			}))
	elif app_id == "text":
		var save_button := window.find_child("TextEditorSaveButton", true, false)
		if save_button is Button and _is_clickable_control_visible(save_button as Button):
			actions.append(_window_action_ref(window_id, app_id, title, focused, "text.save", "Save"))
	elif app_id == "system":
		var connect_button := window.find_child("SystemSettingsBridgeConnect", true, false)
		if connect_button is Button and _is_clickable_control_visible(connect_button as Button):
			actions.append(_window_action_ref(window_id, app_id, title, focused, "system.connect_bridge", "Connect bridge"))
		var disconnect_button := window.find_child("SystemSettingsBridgeDisconnect", true, false)
		if disconnect_button is Button and _is_clickable_control_visible(disconnect_button as Button):
			actions.append(_window_action_ref(window_id, app_id, title, focused, "system.disconnect_bridge", "Disconnect bridge"))
	return actions


func _button_action_refs(root: Node, window_id: String, app_id: String, title: String, focused: bool, mapping: Dictionary) -> Array:
	var actions: Array = []
	for button_text in mapping.keys():
		var button := _find_button_by_text(root, str(button_text))
		if button == null:
			continue
		if not _is_clickable_control_visible(button):
			continue
		var suffix: String = str(mapping[button_text])
		actions.append(_window_action_ref(window_id, app_id, title, focused, suffix, str(button_text)))
	return actions
...
func _find_button_by_text(root: Node, button_text: String) -> Button:
	...
	for child in root.get_children():
		if child is Button:
			...
			if button.text.strip_edges() == button_text:
				return button
		if child is Node:
			var nested: Button = _find_button_by_text(child as Node, button_text)
			if nested != null:
				return nested
	return null


func _is_clickable_control_visible(control: Control) -> bool:
...
func _button_label(button: Button) -> String:
	...
	if button.tooltip_text.strip_edges() != "":
		return button.tooltip_text.strip_edges()
	return button.name


func _perform_os_ui_ref_click(shell: Node, ref: String) -> Dictionary:
	if ref.begins_with("shell.launcher.app."):
		var app_id: String = ref.trim_prefix("shell.launcher.app.")
		if app_id == "":
			return _unsupported_click(ref, "Malformed launcher ref")
		if not shell.has_method("launch_app"):
			return _unsupported_click(ref, "Shell launch_app unavailable")
		var opened: Variant = shell.call("launch_app", app_id)
		if opened == null:
			return {"ok": false, "unsupported": false, "code": "OPEN_FAILED", "message": "Could not open app: %s" % app_id, "ref": ref}
		return {"ok": true, "action": "launcher.open_app", "ref": ref, "app_id": app_id}

	if ref.begins_with("app:") and ref.ends_with(".taskbar.toggle"):
		var app_ref: String = ref.substr(4, ref.length() - 4 - ".taskbar.toggle".length())
		if app_ref == "":
			return _unsupported_click(ref, "Malformed taskbar ref")
		if not shell.has_method("_on_task_button_pressed"):
			return _unsupported_click(ref, "Shell taskbar toggle unavailable")
		shell.call("_on_task_button_pressed", app_ref)
		return {"ok": true, "action": "taskbar.toggle_app", "ref": ref, "app_id": app_ref}

	var separator: int = ref.find(":")
	if separator <= 0:
		return _unsupported_click(ref, "Unsupported ref format")
	var window_id: String = ref.substr(0, separator)
	var action_suffix: String = ref.substr(separator + 1)
	var window: Node = _find_window_from_shell(shell, window_id)
	if window == null:
		return {"ok": false, "unsupported": false, "code": "WINDOW_NOT_FOUND", "message": "Window not found: %s" % window_id, "ref": ref}

	if action_suffix == "window.focus":
		if not shell.has_method("_focus_window"):
			return _unsupported_click(ref, "Shell focus action unavailable")
		shell.call("_focus_window", window)
		return {"ok": true, "action": "window.focus", "ref": ref, "window_id": window_id, "app_id": str(window.get("app_id"))}

	if action_suffix == "window.close":
		if not shell.has_method("_on_window_close_requested"):
			return _unsupported_click(ref, "Shell close action unavailable")
		shell.call("_on_window_close_requested", window)
		return {"ok": true, "action": "window.close", "ref": ref, "window_id": window_id, "app_id": str(window.get("app_id"))}

	var app_id: String = str(window.get("app_id"))
	if action_suffix == "files.new_file":
		return _click_button_in_window(window, "New file", ref, app_id, window_id, "files.new_file")
	if action_suffix == "files.open":
		return _click_button_in_window(window, "Open", ref, app_id, window_id, "files.open")
	if action_suffix == "files.delete":
		return _click_button_in_window(window, "Delete", ref, app_id, window_id, "files.delete")
	if action_suffix == "text.save":
		var save_button := window.find_child("TextEditorSaveButton", true, false)
		if not (save_button is Button):
			return _unsupported_click(ref, "Text save button unavailable")
		return _emit_button_pressed(save_button as Button, ref, app_id, window_id, "text.save")
	if action_suffix == "system.connect_bridge":
		var connect_button := window.find_child("SystemSettingsBridgeConnect", true, false)
		if not (connect_button is Button):
			return _unsupported_click(ref, "Bridge connect button unavailable")
		return _emit_button_pressed(connect_button as Button, ref, app_id, window_id, "system.connect_bridge")
	if action_suffix == "system.disconnect_bridge":
		var disconnect_button := window.find_child("SystemSettingsBridgeDisconnect", true, false)
		if not (disconnect_button is Button):
			return _unsupported_click(ref, "Bridge disconnect button unavailable")
		return _emit_button_pressed(disconnect_button as Button, ref, app_id, window_id, "system.disconnect_bridge")

	return _unsupported_click(ref, "Unsupported ref action")


func _click_button_in_window(window: Node, button_text: String, ref: String, app_id: String, window_id: String, action: String) -> Dictionary:
	var button: Button = _find_button_by_text(window, button_text)
	if button == null:
		return _unsupported_click(ref, "Button unavailable: %s" % button_text)
	return _emit_button_pressed(button, ref, app_id, window_id, action)


func _emit_button_pressed(button: Button, ref: String, app_id: String, window_id: String, action: String) -> Dictionary:
	if not _is_clickable_control_visible(button):
		return _unsupported_click(ref, "Control not visible")
	if button.disabled:
		return _unsupported_click(ref, "Control is disabled")
...
func _find_window_from_shell(shell: Node, window_id: String) -> Node:
	if shell.has_method("_find_window_by_id"):
		var found: Variant = shell.call("_find_window_by_id", window_id)
		if found != null and found is Node and is_instance_valid(found):
			return found as Node
	for window in _visible_windows_for_ui_tree(shell):
		if window == null or not is_instance_valid(window):
			continue
		if _window_id_for_shell(shell, window) == window_id:
			return window
	return null


func _focused_window_summary(shell: Node) -> Dictionary:
	...
	var desktop: Dictionary = state.get("desktop", {}) if state.get("desktop", {}) is Dictionary else {}
	var focused_window_id: String = str(desktop.get("focused_window_id", ""))
	var windows: Array = state.get("windows", []) if state.get("windows", []) is Array else []
	for window_value in windows:
		if not (window_value is Dictionary):
			continue
		var window: Dictionary = window_value
		if str(window.get("id", "")) == focused_window_id:
			var sanitized_window: Variant = _variant_to_json(window)
			if sanitized_window is Dictionary:
				return sanitized_window as Dictionary
			return window.duplicate(true)
	return {}


func _window_id_for_shell(shell: Node, window: Node) -> String:
	if shell != null and shell.has_method("_window_id"):
		return str(shell.call("_window_id", window))
	return "win_%s" % str(window.get_instance_id())


func _is_window_focused(shell: Node, window: Node) -> bool:
...
func _build_os_observe_payload(params: Dictionary = {}) -> Dictionary:
	var shell: Node = _find_hermes_shell()
	var agent_service: Variant = null
	if shell != null and shell.has_method("hermes_agent_service"):
		agent_service = shell.call("hermes_agent_service")

	var max_visible_paths: int = int(params.get("max_visible_paths", 12))
	max_visible_paths = clampi(max_visible_paths, 0, 50)
	var cwd: String = str(params.get("cwd", ""))
	var context_options: Dictionary = {
		"mode": "observe",
		"max_visible_paths": max_visible_paths
	}
	if cwd != "":
		context_options["cwd"] = cwd

	var context: Dictionary = {}
	if agent_service != null and agent_service.has_method("get_context"):
		var context_value: Variant = agent_service.call("get_context", context_options)
		if context_value is Dictionary:
			context = (context_value as Dictionary).duplicate(true)

	var os_block: Dictionary = context.get("os", {}) if context.get("os", {}) is Dictionary else {}
	var apps_block: Array = context.get("apps", []) if context.get("apps", []) is Array else []
	var windows_block: Array = context.get("windows", []) if context.get("windows", []) is Array else []
	var filesystem_block: Dictionary = context.get("filesystem", {}) if context.get("filesystem", {}) is Dictionary else {}
	var bridge_block: Dictionary = context.get("bridge", {}) if context.get("bridge", {}) is Dictionary else {}
	var legacy_block: Dictionary = context.get("legacy_state", {}) if context.get("legacy_state", {}) is Dictionary else {}

	var focused_window: Dictionary = {}
	for window_value in windows_block:
		if not (window_value is Dictionary):
			continue
		var window_dict: Dictionary = window_value
		if bool(window_dict.get("focused", false)):
			focused_window = window_dict.duplicate(true)
			break

	var app_items: Array = []
	for app_value in apps_block:
		if not (app_value is Dictionary):
			continue
		var app_dict: Dictionary = app_value
		var actions_value: Variant = app_dict.get("agent_actions", [])
		var actions_array: Array = actions_value if actions_value is Array else []
		app_items.append({
			"id": str(app_dict.get("id", "")),
			"name": str(app_dict.get("name", app_dict.get("title", ""))),
			"title": str(app_dict.get("title", app_dict.get("name", ""))),
			"category": str(app_dict.get("category", "Other")),
			"pinned": bool(app_dict.get("pinned", false)),
			"single_instance": bool(app_dict.get("single_instance", true)),
			"agent_actions_count": actions_array.size()
		})

	var operation_summary: Dictionary = {
		"supported_operation_count": 0,
		"operations": []
	}
	if shell != null and shell.has_method("hermes_supported_operations") and shell.has_method("hermes_describe_operation"):
		var supported_value: Variant = shell.call("hermes_supported_operations")
		if supported_value is Array:
			var supported_ops: Array = supported_value
			supported_ops.sort()
			operation_summary["supported_operation_count"] = supported_ops.size()
			var operations: Array = []
			for op_value in supported_ops:
				var op_name: String = str(op_value)
				var metadata_value: Variant = shell.call("hermes_describe_operation", op_name)
				var metadata: Dictionary = metadata_value if metadata_value is Dictionary else {}
				operations.append({
					"operation": op_name,
					"capability": str(metadata.get("capability", "legacy.compat")),
			...
			operation_summary["operations"] = operations

	var observe_payload: Dictionary = {
		"os_summary": {
			"name": str(os_block.get("name", "HermesOS")),
			"version": str(os_block.get("version", "dev")),
			"current_user": str(os_block.get("current_user", "")),
			"timestamp": str(os_block.get("timestamp", "")),
			"bridge_connected": bool(bridge_block.get("connected", false))
		},
		"app_summary": {
			"count": app_items.size(),
			"items": app_items
		},
		"window_summary": {
			"count": windows_block.size(),
			"items": windows_block
		},
		"focused_window": focused_window,
		"filesystem_summary": {
			"cwd": str(filesystem_block.get("cwd", "")),
			"home": str(filesystem_block.get("home", "")),
			"current_user": str(filesystem_block.get("current_user", "")),
			"visible_paths": filesystem_block.get("visible_paths", []) if filesystem_block.get("visible_paths", []) is Array else []
		},
		"operation_metadata_summary": operation_summary,
		"bridge_summary": bridge_block,
		"legacy_state_summary": legacy_block,
		"source": {
			"port": PORT,
			"reused_existing_mcp_endpoint": true,
			"shell_available": shell != null,
			"agent_service_available": agent_service != null
		}
	}

	var sanitized: Variant = _variant_to_json(observe_payload)
	if sanitized is Dictionary:
		return sanitized as Dictionary
	return {}


func _find_hermes_shell() -> Node:
	...
	if root == null:
		return null
	if root.has_method("hermes_agent_service"):
		return root
	return _find_node_with_method(root, "hermes_agent_service")
...
func _find_node_with_method(node: Node, method_name: String) -> Node:
	for child in node.get_children():
		...
		if child_node.has_method(method_name):
			return child_node
		var nested: Node = _find_node_with_method(child_node, method_name)
		if nested != null:
			return nested
	return null
