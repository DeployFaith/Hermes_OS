...
func configure_static_buttons() -> void:
	...
	_decorate_static_button("taskbar-notifications", "notification", "Notification history")
	_decorate_static_button("taskbar-user", "user", "Account Center")
...
func _update_indicator(button: Button, task: Dictionary) -> void:
	...
	if bool(task.get("visible", false)):
		indicator.color = Tokens.FOCUS if bool(task.get("active", false)) else Color(Tokens.MUTED.r, Tokens.MUTED.g, Tokens.MUTED.b, 0.45)
