...
func open_file(path: String) -> Dictionary:
	...
	if not bool(read_result.get("ok", false)):
		var error := str(read_result.get("error", "Could not read file"))
	...
	_dirty = false
	_update_path_label()
	_set_status("Opened " + target_path.get_file())
...
func save_file() -> Dictionary:
	...
	return {"ok": ok, "error": message, "path": _current_path}
...
func _build() -> void:
	...
	_path_label.name = "TextEditorPathLabel"
	_path_label.text = "No file opened"
	...
	save_button.name = "TextEditorSaveButton"
	...
	_status_label.name = "TextEditorStatusLabel"
