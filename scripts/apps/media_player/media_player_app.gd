class_name MediaPlayerApp
extends Control

const HermesUIRuntime = preload("res://scripts/ui/hermes_ui/runtime/hermes_ui_runtime.gd")

const MANIFEST_PATH := "res://scripts/apps/media_player/manifest.json"
...
func _build() -> void:
	...
	set_meta("window_min_size", Vector2(480, 380))
	...
	if _instance == null:
		add_child(_error_label("Media Player manifest failed to load."))
		return
	_mounted = _runtime.mount_instance(_instance, self)
	if _mounted == null:
		add_child(_error_label("Media Player runtime failed to mount."))
...
func _error_label(message: String) -> Label:
	var label := Label.new()
	label.name = "MediaPlayerRuntimeError"
	label.text = message
